...
(function() {
  ...
  // Danh sách các mẫu URL cần bỏ qua. Script sẽ không chạy trên các trang này.
  const blacklistedPatterns = [
      "/enhanced-content/",          // Chặn tất cả các trang A+ Content
      "advertising.amazon.com",      // Chặn các trang quảng cáo
      "sellercentral.amazon.com/music", // Chặn trang Music
      "sellercentral.amazon.com/read"   // Chặn trang Read
  ];

  const currentUrl = window.location.href;

  // Kiểm tra xem URL hiện tại có khớp với mẫu nào trong danh sách đen không
  const isPageBlacklisted = blacklistedPatterns.some(pattern => currentUrl.includes(pattern));

  // Nếu trang hiện tại nằm trong danh sách đen, DỪNG THỰC THI SCRIPT NGAY LẬP TỨC.
  if (isPageBlacklisted) {
      // Bạn có thể giữ lại dòng log này để kiểm tra trong Console nếu muốn
      console.log("Bkteam Inject Script: Page is blacklisted, execution stopped.");
      return;
  }

  //-----------------------------------------------------------------------------------
  // LOGIC GỐC CỦA SCRIPT - Chỉ được thực thi nếu trang không bị chặn
  //-----------------------------------------------------------------------------------
  (function (data) {
      const { window } = data;
      const swallow = () => {};
