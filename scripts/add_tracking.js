...
chrome.runtime.onMessage.addListener(async (req, sender, res) => {
   ...
   switch (message) {
      ...
      case "verifyAddTracking": // Message này là từ background.js yêu cầu verify
         console.log("[CS] Received 'verifyAddTracking' (initial request) with data:", data);

         // Bước 1: Gửi ACK sớm cho hàm sendMessage của background
         res({ message: "received" });

         if (!data || !data.orderId) { // Kiểm tra cả orderId nữa cho chắc
         ...
         try { // Bước 2: Thực hiện logic xác minh
         ...
         // Message type này ("verifyAddTracking") phải khớp với `expectedResponseMessage`
         // mà `sendMessageAndPromiseResponse` của background đang chờ.
         chrome.runtime.sendMessage({
            message: "verifyAddTracking", // Hoặc "verifyAddTrackingResult" nếu mày muốn đổi và sửa cả ở background
            data: verificationPayload,
            domain: window.location.origin
         });

         // Bước 4: Return true vì đã có res() và chrome.runtime.sendMessage()
         return true;
...
const handleTracking = async (trackingInfo, confirmType) => {
   ...
   try {
      ...
      if (trackingCode !== "") {
         ...
         // carrierNativeSelect.focus(); // Optional: sometimes helps
         // carrierNativeSelect.blur();  // Optional: sometimes helps
         ...
         if ($promptSpan.length) {
            ...
            // Handle truncation (e.g., "Canadian Warehouse ..." vs "Canadian Warehouse And Logistics")
         ...
         if (isOtherCarrier && carrier) { // This 'carrier' is the original target carrier name (e.g., "MyCustomCarrier")
      ...
      if (confirmType === 'add') {
         ...
         if (trackingCode === "") {
            await sleep(1000); // Wait for potential dialog
            try {
         ...
         while (editRetries > 0) {
            ...
            // Thử tìm nút "YES, please continue..."
      ...
      // 5. Send message (sau khi các hành động click đã hoàn tất và có thời gian chờ)
      chrome.runtime.sendMessage({
      ...
      // 6. Wait for alert heading (final confirmation of page state)
