...
// Đợi ô Label xuất hiện (sau khi đã tạo customization)
async function waitForLabelInput() {
  return waitFor(() => deepQuerySelector(SEL_LABEL_INPUTS), { timeout: 20000, interval: 150 });
}

// Gõ label và bắn event để UI nhận
async function setGroupLabel(labelText) {
  ...
  input.value = labelText || "Choose Book";
  input.dispatchEvent(new InputEvent("input", { bubbles: true }));
  input.dispatchEvent(new Event("change", { bubbles: true }));
  input.blur?.();
  cslog(`Set label to "${labelText}"`);
  return input;
}
  ...
  if (msg?.type === "NCNAS_APPLY") {
    (async () => {
      try {
        ...
        // NEW: Gõ label rồi bấm Add option trong đúng container đang mở
        const labelInput = await setGroupLabel(targetLabel);
        // đợi UI vẽ lại tiêu đề/khung
        await delay(300);
